...
COPY application.yml /config/application.yaml

# 8. Spring Boot 애플리케이션 실행
ENTRYPOINT ["java", "-jar", "/app.jar", "--spring.config.additional-location=/config/application-secret.properties", "--spring.config.additional-location=/config/application.yaml"]
