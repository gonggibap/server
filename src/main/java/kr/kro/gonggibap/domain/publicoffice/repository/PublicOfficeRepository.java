...
public interface PublicOfficeRepository extends JpaRepository<PublicOffice, Long> {
    ...
    @Query(value = "SELECT new kr.kro.gonggibap.domain.restaurant.dto.response.RestaurantResponse(r.id, r.restaurantName, r.phone, r.link, r.category, r.detailCategory, r.addressName, r.roadAddressName, r.latitude, r.longitude, h.publicOffice.id, p.name, COUNT(h), AVG(rev.point)) " +
            "FROM History h " +
            "LEFT JOIN h.restaurant r " +
